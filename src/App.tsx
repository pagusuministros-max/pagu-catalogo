...
// --- PRODUCTOS PAGÚ (Aumento 15% incluido) ---
const categories: Category[] = [
  {
    ...
    products: [
      ...
      { id: 20003, name: "Carpeta 3 Solapas Colores (x12)", price: 24564, img: "https://images.unsplash.com/photo-1621360841013-c7683c659ec6?q=80&w=800" },
    ],
  },
  {
    title: "Papelería & Separadores",
    products: [
      { id: 20000, name: "Separadores Nº3 (x6 hojas) 230g", price: 1610, img: "https://images.unsplash.com/photo-1600000882773-a630f5d139ca?q=80&w=800" },
  ...
  },
  {
    ...
    products: [
      ...
      { id: 7025, name: "Set Libreta Full Accesorios (x12)", price: 54000, img: "https://images.unsplash.com/photo-1531346878377-a5be20888e57?q=80&w=800" },
  ...
  },
  {
    ...
    products: [
      ...
      { id: 7066, name: "Gomas Unicornios Marino (x12)", price: 24840, img: "https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=800" },
      { id: 7063, name: "Goma Animalitos (x24)", price: 24840, img: "https://images.unsplash.com/photo-1541829070764-84a7d30dee62?q=80&w=800" },
      { id: 7062, name: "Sacapuntas con Goma (x12)", price: 21600, img: "https://images.unsplash.com/photo-1550684848-fac1c5b4e853?q=80&w=800" },
    ],
  },
];
...
export default function App() {
  const [cart, setCart] = useState<(Product & { q: number })[]>([]);
  ...
  return (
    <div className="bg-white text-black min-h-screen font-sans antialiased">
      <header className="py-16 text-center border-b border-gray-50">
        <h1 className="text-6xl md:text-8xl font-light tracking-tighter">PAGÚ</h1>
      ...
      <button onClick={() => setIsCartOpen(true)} className="fixed bottom-6 right-6 z-50 p-4 bg-black text-white shadow-2xl rounded-full flex items-center gap-2">
        ...
        {cart.length > 0 && <span className="font-bold text-xs border-l pl-2">{cart.reduce((a,b)=>a+b.q, 0)}</span>}
      </button>
        ...
        {categories.map((cat, idx) => (
          <section key={idx} className="mb-20">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-10">
              {cat.products.map(p => (
                <div key={p.id} className="flex flex-col group cursor-pointer" onClick={() => addToCart(p)}>
                  ...
                  <button className="mt-2 text-[9px] uppercase font-bold border border-black py-2 hover:bg-black hover:text-white transition-colors">Añadir</button>
                </div>
              ))}
            </div>
          </section>
      ...
      <AnimatePresence>
        {isCartOpen && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-black/40 backdrop-blur-sm flex justify-end">
            <motion.div initial={{ x: "100%" }} animate={{ x: 0 }} exit={{ x: "100%" }} className="w-full max-w-md bg-white h-full p-6 flex flex-col shadow-2xl">
              <div className="flex justify-between items-center mb-8">
                <h3 className="text-sm font-bold uppercase tracking-widest">Resumen</h3>
                <button onClick={() => setIsCartOpen(false)}><X size={24} /></button>
              </div>
              <div className="flex-1 overflow-y-auto space-y-6">
                {cart.length === 0 ? <p className="text-center text-gray-400 text-xs py-20 uppercase tracking-widest">Sin productos</p> : 
                  cart.map(i => (
              ...
              <div className="pt-6 border-t mt-auto">
                <textarea placeholder="Notas adicionales..." className="w-full p-3 bg-gray-50 text-xs mb-4 rounded border-none outline-none" rows={2} onChange={(e) => setNote(e.target.value)} />
                <div className="flex justify-between text-sm font-bold mb-6 uppercase tracking-widest">
                  <span>Total</span>
                  <span>{formatPrice(total)}</span>
                </div>
                <button onClick={sendWhatsApp} disabled={cart.length === 0} className="w-full bg-green-500 text-white py-4 rounded font-bold flex items-center justify-center gap-2 hover:bg-green-600 disabled:bg-gray-200 uppercase text-xs tracking-widest">
